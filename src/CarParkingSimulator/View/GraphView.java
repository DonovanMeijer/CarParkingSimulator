...
public class GraphView extends JPanel
{
    ...
    public GraphView(ArrayList<Payment> revenue)
    {
    ...
    public void paintComponent(Graphics g) {
        ...
        double xScale = ((double) getWidth() - 2 * BORDER_GAP) / (revenue.size() - 1);
        double yScale = ((double) getHeight() - 2 * BORDER_GAP) / (MAX_SCORE - 1);

        ArrayList<Point> graphPoints = new ArrayList<Point>();
        int paymentStep = 0;
        double totalAmount = 0;
        for(Payment payment : revenue)
        {
        ...
        for (int i = 0; i < revenue.size() - 1; i++) {
            int x0 = (i + 1) * (getWidth() - BORDER_GAP * 2) / (revenue.size() - 1) + BORDER_GAP;
            int x1 = x0;
    ...
    public static void createAndShowGui() {
        GraphView mainPanel = new GraphView(revenue);
